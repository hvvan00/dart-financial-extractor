...
항목은 행 번호가 아니라 항목명을 기준으로 맞춥니다. 앞의 번호, 공백, 끝의
주석번호만 달라진 경우에는 같은 항목으로 처리합니다. 반대로 계정 이름 자체가
바뀐 경우에는 억지로 합치지 않고 각각 별도 행으로 남기므로, 이름이 달랐던
연도에는 빈칸이 표시됩니다. 새로운 항목이나 사라진 항목도 같은 방식으로
보존합니다.

비교 표시 때문에 같은 연도가 여러 공시에 겹칠 수 있습니다. 이 경우 더 최근에
제출된 공시에 표시된 해당 연도 금액을 사용합니다. 이는 과거 수치가 재분류되거나
재작성된 경우 최신 보고서의 비교 금액을 반영하기 위한 동작입니다.
