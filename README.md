...
오래된 PDF에서 상위 `매출액` 행이 빠지고 `제품매출액`, `상품매출액`,
`용역매출액` 같은 세부항목만 추출된 경우에도 항목의 의미로 `매출액` 구간을
복원합니다. 다른 연도의 `제품매출`, `상품매출`, `용역매출`과 각각 한 행으로
합치며, 총계인 `매출액`과 세부 매출을 서로 합치지는 않습니다.
해당 연도의 `매출액` 또는 `매출원가` 총계 금액만 비어 있고 직접 하위항목의
금액이 중복 없이 확인되면 그 하위항목을 합산하여 빈 총계를 숫자로 채웁니다.
공시에 이미 있는 총계는 덮어쓰지 않으며, 같은 하위항목이 중복 추출된 경우에는
잘못된 이중 합산을 피하기 위해 자동 계산하지 않습니다.
...
보고서마다 달라지는 `Ⅰ.`, `(3)`, `1.`, `가.` 같은 원문 순번은 최종 Excel
항목명에서 제거합니다. 여러 연도를 합쳤을 때 일부 연도 항목에만 번호가 남아
형제 항목이 하위 항목처럼 보이는 문제를 막기 위한 처리입니다. 같은 단계의
항목은 같은 들여쓰기로 표시합니다. 이 규칙은 특정 자산 구간에만 적용되는 것이
아니라 재무상태표·손익계산서·현금흐름표 전체에 적용됩니다. 예를 들어 손익계산서의
`제품매출원가`와 `상품매출원가`, 현금흐름표의 활동별 `현금유입액`과
`현금유출액`은 원문의 순번 모양이 달라도 같은 단계와 같은 색으로 표시합니다.
재무상태표의 `대손충당금`,
`감가상각누계액`, `정부보조금`, `사채할인발행차금`처럼 바로 앞 계정에
종속되는 차감항목만 한 단계 더 들여쓴 `└ 항목명`으로 표시합니다.
